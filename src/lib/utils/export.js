 ...
 */
export async function exportCapTableToCSV(founders, rounds, employees, optionPool, currentDate) {
  try {
...
export async function exportToPDF(company, founders, rounds, employees) {
  try {
    ...
    pdf.text(company.name || 'Equity Calculator Report', pageWidth / 2, yPosition, { align: 'center' })

    yPosition += 15
    pdf.setFontSize(12)
    pdf.setTextColor(100, 100, 100)
    pdf.text(`Generated: ${format(new Date(), 'MMMM dd, yyyy')}`, pageWidth / 2, yPosition, { align: 'center' })
    ...
    pdf.text(`Founded: ${format(new Date(company.foundedDate), 'MMMM dd, yyyy')}`, margin, yPosition)
    yPosition += 6
    ...
    const disclaimerText = 'This is a financial model and should not be considered legal or financial advice. ' +
    ...
    if (ownershipChartImg) {
      ...
      const imgHeight = 80
      checkPageBreak(imgHeight + 10)
      pdf.addImage(ownershipChartImg, 'PNG', (pageWidth - imgWidth) / 2, yPosition, imgWidth, imgHeight)
      yPosition += imgHeight + 10
    }
    ...
    pdf.setTextColor(150, 150, 150)
    pdf.text(
      'Generated by Founder Equity Calculator',
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    )
